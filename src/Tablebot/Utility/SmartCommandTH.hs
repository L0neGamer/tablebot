...
choiceAccessor x
  ...
  where
    var = mkName "x"
    decName = mkName $ "Choice" ++ show x
    rtsThenLfts = xRightsThenLeft x
    decl :: Dec
    decl = PatSynD decName (PrefixPatSyn [var]) (ExplBidir cs) rtsThenLfts
    ...
    xRightsThenLeftExp :: Int -> Exp
    xRightsThenLeftExp 0 = AppE (ConE $ mkName "LeftS") (VarE var)
    xRightsThenLeftExp n = AppE (ConE $ mkName "RightS") (xRightsThenLeftExp (n - 1))
    cs :: [Clause]
    cs = pure $ Clause ([VarP var]) (NormalB (xRightsThenLeftExp x)) []
