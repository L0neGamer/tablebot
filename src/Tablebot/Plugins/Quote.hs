...
quoteComm ::
  WithError
    "Unknown quote functionality."
    ( AnyOf
        '[ (Exactly "add", Quoted Text, Exactly "-", RestOfInput Text),
           (Exactly "show", Int),
           (Exactly "delete", Int)
         ]
    ) ->
...
-- quoteComm (WErr (LeftS (_,Qu _,_,_))) = undefined
-- quoteComm (WErr (RightS (LeftS _))) = undefined
-- quoteComm (WErr (RightS (RightS (LeftS _)))) = undefined
